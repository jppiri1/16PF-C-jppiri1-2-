# -*- coding:utf8 -*-
# 상위 클래스 이름이너무 길어져서 from import as 적용
...
class SafeCashCard(CashCard):
    # SimpleCashCard의 다른기능은 그대로 사용하고
    # {__init__(), deposit(), check_balance()}
    # withdraw()메소드만 다시 정의한다
    def withdraw(self, amount_won):
        """
        SafeCashCard withdraw methd
        Check balance before withdraw
        """
        print("SafeCashCardwithdraw()") # 함수 호출 표식
        # 잔고가 충분하면
...
# 아래의 내용은 이 파일이 import 될 때는실행되지 않음
if "__main__" == __name__:
    # Cash Card User 모듈의 msg_int 함수를 사용할수 있게 함
    ...
    chk_bal("mySisterSafeCard 출금 후 잔고 확인", mySistersSafeCard)
